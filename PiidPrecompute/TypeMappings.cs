...
public static class TypeMappings
{
    ...
    public static bool TryGetMappedType(string fqn, [NotNullWhen(true)] out string? mappedType)
    {
        if (_netTypeMappings.TryGetValue(fqn, out var mapping))
        {
            mappedType = mapping.Fqn;
            return true;
        }
...
public record struct TypeMapping
{
    ...
    /// </summary>
    public readonly string Fqn => $"{TypeNamespace}.{TypeName}";
}