...
public readonly record struct AhkParameter
{
    ...
    private string GetName()
    {
        ...
        {
            Trace.TraceInformation($"Indexing type names for assembly '{mr.GetAssemblyDefinition().GetAssemblyName().Name}'");
            ...
            Trace.TraceInformation($"Finished in {watch.Elapsed.TotalMilliseconds} ms");

            IndexedAssemblies.Add(mr);
        }

        bool isNameReserved = ReservedNames.Retrieve(nameVal)
            .Any(n => n.Equals(nameVal, StringComparison.InvariantCultureIgnoreCase));

        return isNameReserved ? mr.GetString(param.Name) + "_" : mr.GetString(param.Name);
    }
